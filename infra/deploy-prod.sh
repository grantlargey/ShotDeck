...
API_HEALTH_URL="${API_HEALTH_URL_OVERRIDE:-http://shotdeck-lb-1634199451.us-east-1.elb.amazonaws.com/health}"
API_SMOKE_URL="${API_SMOKE_URL_OVERRIDE:-http://shotdeck-lb-1634199451.us-east-1.elb.amazonaws.com/movies}"
...
if [[ "$deploy_frontend" -eq 1 ]]; then
  ...
  aws s3 cp "$ROOT_DIR/client/dist/index.html" "s3://$FRONTEND_BUCKET/404.html" \
    --cache-control "no-cache,no-store,must-revalidate" \
    --content-type "text/html; charset=utf-8" \
    --region "$AWS_REGION"
